...
class MedicalImage(db.Model):
    ...
    def to_dict(self, include_annotations=False):
        ...
        if include_annotations:
            data['annotations'] = [
                a.to_dict() for a in self.annotations.all()
            ]
...
class Annotation(db.Model):
    ...
    image_id = db.Column(
        db.String(36),
        db.ForeignKey('medical_images.id'),
        nullable=False,
        index=True
    )

    label_class_id = db.Column(
        db.String(36),
        db.ForeignKey('label_classes.id')
    )
...
class AIInferenceResult(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'segmentations': json.loads(self.segmentations)
            if self.segmentations else [],
